  ...
  class Program
  {
    ...
    public static void ReturnTypeItem(ref Items.Itens[] itens, int i)
    {
      ...
      itens[i].item = iReturnTypeItem == 1 ? Items.eTypeOfItem.shirt :
        ...
        iReturnTypeItem == 6 ? Items.eTypeOfItem.shorts : Items.eTypeOfItem.checkout;
    ...
    public static int ItensScreen()
    {
      ...
      while (!bValidate)
      {
        ...
        for (int i = 1; i <= Items.eTypeOfItem.checkout.GetHashCode(); i++)
        {
          iLine += 1;
          Console.SetCursorPosition(LEFTPOSITIONSCREEN + 15, iLine);
          Console.Write("[{0}] - {1}", i, i != Items.eTypeOfItem.checkout.GetHashCode() ? "Buy - " + Items.TakeTypeOfItemByNumber(i) : "Checkout");
        }
        ...
        {
          MesssageAlertScreen("Please select one of the available options = 1 to " + Items.eTypeOfItem.checkout.GetHashCode() + "!!!");
        }
        else if (int.Parse(sInputAnswer) > Items.eTypeOfItem.checkout.GetHashCode())
        {
          MesssageAlertScreen("Please select one of the available options = 1 to " + Items.eTypeOfItem.checkout.GetHashCode() + "!!!");
        }
        else
      ...
      iLine = 5;
      for (int i = 1; i <= Items.eTypeOfItem.checkout.GetHashCode() + 1; i++)
      {
        ReDrawLine(iLine);
        iLine += 1;
      }
